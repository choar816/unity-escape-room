...
public class Drawable : MonoBehaviour
{
    ...
    public static Drawable drawable; // TODO : 드로우보드 여러개라 static 풀어야 함
    ...
    void Update()
    {
        if (ControlCenter.Instance.gameInfo.mode == ControlCenter.GameMode.Thought_Draw)
        {
    ...
    public Vector2 WorldToPixelCoordinates(Vector2 world_position)
    {
        // Convert the world position directly to drawable_sprite's local position
        SpriteRenderer spriteRenderer = GetComponent<SpriteRenderer>();

        Vector3 localPosition = spriteRenderer.transform.InverseTransformPoint(world_position);

        // Convert to pixel coordinates
        float pixelWidth = drawable_sprite.rect.width;
        float pixelHeight = drawable_sprite.rect.height;

        float unitsToPixels = pixelWidth / drawable_sprite.bounds.size.x;  // This considers only the drawable_sprite's scale
    ...
    public void Init(Sprite _sprite)
    {
        ...
        drawable_sprite = _sprite;
        drawable_texture = _sprite.texture;
